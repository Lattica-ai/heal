...
TEST(GDecompositionEdgeCases, ScalarValues) {
    ...
    apply_g_decomp<int32_t>(a_hw, power, base_bits, result_hw);
...
TEST(GDecompositionEdgeCases, ZeroInput) {
    ...
    apply_g_decomp<int32_t>(a_hw, 4, 2, result_hw);  // base = 4
...
TEST(GDecompositionEdgeCases, MultiDimensionalInput) {
    ...
    apply_g_decomp<int32_t>(a_hw, 3, 2, result_hw);  // base = 4
...
TEST(GDecompositionEdgeCases, OverflowWarningCheck) {
    ...
    apply_g_decomp<int32_t>(a_hw, 3, 3, result_hw);  // base = 8, max representable = 512
...
TEST(GDecompositionEdgeCases, InvalidShapeMismatch) {
    ...
    EXPECT_THROW(
        apply_g_decomp<int32_t>(a_hw, 2, 2, result_hw),
