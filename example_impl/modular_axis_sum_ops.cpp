...
void validate_modmul_inputs(
    const std::shared_ptr<DeviceTensor<T>>& a,      // [reps, sum_size, p_list_len, n]
    const std::shared_ptr<DeviceTensor<T>>& b,      // [      sum_size, p_list_len, n]
    const std::shared_ptr<DeviceTensor<T>>& p,
    const std::shared_ptr<DeviceTensor<T>>& perm,
    const std::shared_ptr<DeviceTensor<T>>& result, // [reps,           p_list_len, n]
    bool apply_perm,
    ...
    std::shared_ptr<DeviceTensor<T>>& result)
{
    ...
    validate_modmul_inputs(a, b, p, perm, result, apply_perm, reps, n, sum_size, k);
